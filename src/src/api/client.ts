...
const demoResponse = (method: string, url: string, body?: unknown): ApiResponse<unknown> | null => {
  ...
  if (url === "/user/notifications") return ok({ total: 3, notifications: [
    { id: 1, title: "晚间呼吸练习", content: "今晚 21:30 有一段 5 分钟训练", read: false },
    { id: 2, title: "咨询提醒", content: "明天下午有一场预约咨询", read: false },
    { id: 3, title: "测评建议", content: "可以复测 GAD-7 观察近期变化", read: true }
  ] });
  ...
  if (url === "/assessment/scales") return ok({ scales: [
    { scaleKey: "PHQ9", title: "PHQ-9 抑郁筛查", description: "9 个问题，帮助观察近期情绪低落程度" },
    { scaleKey: "GAD7", title: "GAD-7 焦虑筛查", description: "7 个问题，评估焦虑频率与影响" },
    { scaleKey: "PSS10", title: "PSS-10 压力量表", description: "了解近期主观压力水平" }
  ] });
  ...
  if (url === "/appointment/available-slots") return ok({ date: "2026-05-18", slots: [
    { startTime: "2026-05-18 14:00:00", endTime: "2026-05-18 15:00:00", available: true },
    { startTime: "2026-05-18 16:00:00", endTime: "2026-05-18 17:00:00", available: true },
    { startTime: "2026-05-18 19:30:00", endTime: "2026-05-18 20:30:00", available: false }
  ] });
...
export const createChatStream = async (sessionId: string, content: string) => {
  if (isDemo()) {
    const chunks = [
      "我听见你提到：",
      content || "现在有些压力",
      "。我们先把这件事拆小一点，给身体一个可以执行的下一步。"
    ];
